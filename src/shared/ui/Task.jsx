...
const Task = ({iteration_id, task}) => {
  ...
  const handleDeleteTask = (id) => {
    dispatch(deleteTask({iteration_id: iteration_id.iteration_id, id}))
  }
    ...
    <li>
      <form className='py-3 h-14 flex justify-between border-b-[1px] border-gray-800' onSubmit={formik.handleSubmit}>
        <div className='flex items-center'>
          <input onChange={() => handleCheckboxChange(task.id)} checked={task.is_finished} id="default-checkbox" type="checkbox" value="" className="w-5 h-5 text-[#1f99f5] bg-gray-300 cursor-pointer border-gray-300 rounded focus:ring-blue-300"/>
